...
public class EasyImage implements Constants {
    ...
    public interface Callbacks {
        ...
        void onImagesPicked(@NonNull List<File> imageFiles, ImageSource source, int type) throws IOException;
    ...
    private static Intent createAllDocumentsIntent(@NonNull Context context, int type) {
        storeType(context, type);
        Intent intent = new Intent(Intent.ACTION_GET_CONTENT);
        intent.setType("*/*");
    ...
    private static Intent createChooserIntent(@NonNull Context context, @Nullable String chooserTitle, boolean showGallery, int type) throws IOException {
        ...
        if(type==1){
            cameraIntents.add(createAllDocumentsIntent(context, type));
        }
    ...
    private static File takenCameraPicture(Context context) throws IOException, URISyntaxException {
        ...
        if (lastCameraPhoto != null) {
            return decodeFile(lastCameraPhoto);
        } else {
            return null;
        }
    }
    private static File decodeFile(String path) {
        try {
